...
class Agent(nn.Module):
    def __init__(self, num_actions):
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            self._layer_init(nn.Linear(256 * 8 * 8, 512)),
            nn.ReLU(),
        )
...
if __name__ == "__main__":
    ...
    optimizer = optim.Adam(agent.parameters(), lr=0.00009, eps=1e-6)
    ...
    default_filename = f"trained_mic_{total_episodes}_epochs.pk1"
    ...
    if args.model_path:
        ...
        for episode in range(total_episodes):
            ...
            with torch.no_grad():
                ...
                action_histories = [None] * env.num_agents
                ...
                for step in range(0, max_cycles):
                    ...
                    # update action histories for each agent
                    for agent_idx, agent_action in enumerate(actions):
                        if action_histories[agent_idx] is None:
                            action_histories[agent_idx] = agent_action.unsqueeze(0)
                        else:
                            action_histories[agent_idx] = torch.cat((action_histories[agent_idx], agent_action.unsqueeze(0)), dim=0)
                    ...
                    if any([terms[a] for a in terms]) or any([truncs[a] for a in truncs]):
                        end_step = step
                        bonus_reward = min(1, 20 / end_step)
                        total_episodic_return += bonus_reward
                        rb_rewards[end_step] += bonus_reward

                        break
                    
            # Compute MIC rewards and add them to the original rewards
            mic_rewards_dict = {}
            agent_idx = 0
            for agent_key, agent_rewards in rewards.items():
                agent_mic_rewards = []
                for partner in range(num_agents):
                    if agent_idx != partner:
                        mine = MINE(alpha=.6, c=15, est="mic_approx")
                        mine.compute_score(action_histories[agent_idx].cpu(), action_histories[partner].cpu())
                        score = mine.mic()
                        agent_mic_rewards.append(score)
                    else:
                        agent_mic_rewards.append(0.0)
                mic_rewards_dict[agent_key] = agent_mic_rewards  # Use the same string key as in the rewards dictionary
                agent_idx +=1

            for actor in mic_rewards_dict:
                mic_rew = (sum(mic_rewards_dict[actor])/(num_agents-1))
                rewards[actor] += mic_rew
                total_episodic_return += mic_rew
                rb_rewards[end_step] += mic_rew
...
""" RENDER THE POLICY """
env = pistonball_v6.parallel_env(render_mode="human", continuous=False) # for watching
# env = pistonball_v6.parallel_env(render_mode="rgb_array", continuous=False) # for saving
env = color_reduction_v0(env)
...
with torch.no_grad():
    ...
    for episode in range(evals):
        ...
        while not any(terms) and not any(truncs):
         ...
         # Capture and store the frame comment out next
            # frame = np.array(env.render())
            # frames.append(frame)
        # Save the frames as a gif
        # imageio.mimsave('pistonball_untained.gif', frames, fps=24) # comment out up to here if no save
...
path_to_file = f"actions_hist_trained_mic_{evals}_{total_episodes}.pkl"
